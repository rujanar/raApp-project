    ...
    <body>
		<div class="container">
                    ...
                    <div class="content">
                        ...
                        <h3>Pregled Vaših završenih aktivnosti</h3>
                        </div>
				...
				<tr>
					...
					<th>&nbsp;&nbsp;Status&nbsp;&nbsp;</th>
                                ...
                                <c:forEach items="${trenutniZadaci}" var="zadatak">
                                    ...
                                    <c:forEach items="${aktivnosti}" var="aktivnost">
                                        <c:if test="${aktivnost.akt_zadatak_id==zadatak.zadatak_id}">
                                            <c:if test="${aktivnost.status=='Završeno'}">
                                            <% counter++;%>
                                            </c:if>
                                        </c:if>
                                    </c:forEach>
                                ...
                                <c:forEach items="${trenutniZadaci}" var="zadatak">
                                    <tr>
                                        ...
                                        <c:if test="${aktivnost.akt_zadatak_id==zadatak.zadatak_id}">
                                            ...
                                            <!-- status aktivnosti -->
                                            <td class='td-centar-zad' style="background-color: #FFD9E0;"><c:out value="${aktivnost.status}" /></td>
                                    </tr>
                                        </c:if>
                                        </c:if>
                                    </c:forEach> 
                        ...
                        <c:if test="${empty trenutniZadaci}">
                            <h4>Nemate aktivnosti koje su završene.</h4>
                        </c:if>
                        </div>
		</div>
	</div>
